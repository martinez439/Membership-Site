<?php
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(empty($email_err) && empty($password_err)){
      // Prepare query
      $sql = 'SELECT name, email, password, plan_type FROM users WHERE email = :email';
          ...
          if($stmt->rowCount() === 1){
            if($row = $stmt->fetch()){
              ...
              if(password_verify($password, $hashed_password)){
                ...
                $_SESSION['plan_type'] = $row['plan_type'];
                if($_SESSION['plan_type']=="ProPro"){
               header('location: helloworld.php');
                } elseif ($_SESSION['plan_type']=='' || $_SESSION['plan_type']=='basic_1')
